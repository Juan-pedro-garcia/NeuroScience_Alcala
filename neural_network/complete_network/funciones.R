...
conexiones <- function( pres,post, fuerza,nc){ 
  ...
  nc_post <- nc*post                                         #cuantas conexiones reciben las postsinapticas en total
  con_total <- round(runif(1,nc_post-1,nc_post+1))           #varía el numero de conexiones para aumenar la variabilidad entre circuitos.
  con_pres <- rep(con_total%/%pres,pres)                     #cuantas conexiones reciben las presinapticas en total
  con_pres[sample(pres,con_total%%pres)] <- con_pres[sample(pres,con_total%%pres)]+1
  i=0
  while (any(con_pres>0)) {                                  #va estableciendo conexiones de una en una de forma que sean homogéneas
    if(i==post){i=0}
    i=i+1
    
    if(length(which(con_pres>0))>1){s <- sample(which(con_pres>0),1)
    }else{s <- which(con_pres>0)}
    matriz[s,i] <- fuerza
    con_pres[s] <- con_pres[s]-1
  }
  return <- matriz
}
...
calculo_frecuencia<- function(bin=0.200,              #bin <- float; tiempo previo al disparo que se usa para calcular la frecuencia
  ...
  for (h in 1:(length(grupo))) {
    ...
    for (i in 0:(n-1)) {
      ...
      l=0
      if (i!=(n-1)){m <- (i*1000+2):((i+1)*1000)}else{m <-(i*1000+2):(length(conj_dispa))} #seleccion el bloque de 1000 disparos.
...
calculo_maximos_eventos <- function(frame_freq_todo,limite){  #limite <- float; umbral a partir del cual se considera que se produce un evento.
  ...
  while (i<(nrow(frame_freq_todo))) {
    print(paste(i,"de",nrow(frame_freq_todo)))
    superior <- c()
    tiempo <- c()
